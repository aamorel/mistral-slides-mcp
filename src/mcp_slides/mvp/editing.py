...
def normalize_element(element: dict, grouped: bool = False) -> dict:
    ...
    if kind == "shape":
        ...
        try:
            if grouped or not re.fullmatch(r"mvp_(title|body)_\d+", element["objectId"]):
...
longer than its max_characters limit. Return strict JSON, no other fields or commentary.
"""
...
def edit_deck_slide(creds: Any, presentation_id: str, slide_id: str,
    ...
    targets = {e["element_id"]: {"element_id": e["element_id"], "paragraphs": e["paragraphs"],
               "max_characters": [max(len(p), 100 if e["element_id"].startswith("mvp_title_") else 180) for p in e["paragraphs"]]}
